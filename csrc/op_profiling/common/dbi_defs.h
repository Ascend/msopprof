...
constexpr uint64_t SIMT_THREAD_GAP = 64U; // 每个thread间需要间隔64B
constexpr uint64_t MAX_BLOCK = 108U; // 支持的最大block数为108
constexpr uint64_t MAX_BLOCK_DATA_SIZE = 100U * 1024 * 1024; // 每个block 100MB
...
struct BlockHeader {
    ...
    uint64_t loopSizeOuttol1;   // #3707 set_loop_size_para_outtol1，MOV_OUT_TO_L1_ALIGN_V2指令使用
    uint64_t loop1StrideOuttol1;   // #3694 set_loop1_stride_para_outtol1，MOV_OUT_TO_L1_ALIGN_V2类指令使用
    uint64_t loop2StrideOuttol1;   // #3699 set_loop2_stride_para_outtol1，MOV_OUT_TO_L1_ALIGN_V2类指令使用
