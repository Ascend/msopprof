...
std::unordered_map<Common::SimtType, std::string> SIMT_TYPE_MAP {
    ...
    {Common::SimtType::ST, "ST"},
    {Common::SimtType::LDG, "LDG"},
    {Common::SimtType::STG, "STG"},
    {Common::SimtType::LDS, "LDS"},
    {Common::SimtType::STS, "STS"},
    {Common::SimtType::LDK, "LDK"},
    {Common::SimtType::STK, "STK"},
    {Common::SimtType::ATOM, "ATOM"},
    {Common::SimtType::RED, "RED"},
};
...
std::ostream &operator<<(std::ostream &os, const SimtType& simtType)
{
    if (SIMT_TYPE_MAP.find(simtType) == SIMT_TYPE_MAP.end()) {
        return os << "unknown";
    }
    return os << SIMT_TYPE_MAP[simtType];
}
...
{
    auto it = OperandTypeStrMap.find(operandType);
    if (it == OperandTypeStrMap.end()) {
        return os << "unknown";
    }
    return os << it->second;
}
...
std::ostream &operator<<(std::ostream &os, LoadStoreRecord const &record)
{
    return os << "core:" << record.coreID
              << ";" << "pc:0x" << std::hex << record.pc << std::dec
              << ";" << "reg:0x" << std::hex << record.reg << std::dec
              << ";" << "addr:0x" << std::hex << record.addr << std::dec
              << ";" << "memType:" << record.memType
              << ";" << "memSize:" << record.size
              << ";" << "dataType:" << record.dataType
              << ";" << "simtType:" << record.simtType;
}

std::ostream &operator<<(std::ostream &os, AtomRedRecord const &record)
{
    return os << "core:" << record.coreID
              << ";" << "pc:0x" << std::hex << record.pc << std::dec
              << ";" << "addr:0x" << std::hex << record.addr << std::dec
...
template void DBIHelper::PrintRecord<LoadStoreRecord>(const LoadStoreRecord &) const;
template void DBIHelper::PrintRecord<AtomRedRecord>(const AtomRedRecord &) const;
}
