...
class ParseTimeline {
public:
    inline std::vector<TimelineInfo> GetTimeline() {return timelineVec_;}
...
private:
    ...
    std::vector<TimelineInfo> timelineVec_;
    ...
    // endMarks用来屏蔽workaround打点 channelId -> {mark index, len}  len表示连在一起的stamp mark数量
    std::unordered_map<uint32_t, std::pair<uint64_t, uint64_t>> endMarks_;
};
}
#endif