...
```text
"mindstudio-opprof_<version>_<arch>.run" successfully created.
```
...
> 
> - 如果要指定路径安装，则需添加```--install-path```，例如```./mindstudio-opprof_<version>_<arch>.run  --install-path=./test --run```，则将此run包安装到当前目录下的test目录下。
...
> [!NOTE]  说明  
> 默认会升级到${HOME}/Ascend目录下的mindstudio-opprof，如果老版本是通过指定路径安装的，则需添加```--install-path```，例如```./mindstudio-opprof_<version>_<arch>.run  --install-path=./test --run```，其中test是老版本的安装路径。
...
> [!NOTE]  说明  
> 默认会在${HOME}/Ascend目录下卸载，如果安装时通过```--install-path```指定了安装路径，则卸载时也需添加```--install-path```，例如```./mindstudio-opprof_<version>_<arch>.run  --install-path=./test --uninstall```。
