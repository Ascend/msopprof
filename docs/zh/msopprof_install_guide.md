...
```text
"mindstudio-opprof_<version>_linux_<arch>.run" successfully created.
```
...
1. 安装软件包前需给run包添加可执行权限。进入run包保存路径，执行如下命令，增加可执行权限。
    ...
    ```shell
    chmod +x mindstudio-opprof_<version>_linux_<arch>.run
    ```
 
2. 将run包拷贝到运行环境中，执行以下命令安装。
 
    ```shell
    ./mindstudio-opprof_<version>_linux_<arch>.run --run   
    ```
...
> 
> - 如果要指定路径安装，则需添加```--install-path```，例如```./mindstudio-opprof_<version>_linux_<arch>.run  --install-path=./test --run```，则将此run包安装到当前目录下的test目录下。
...
> [!NOTE]  说明  
> 默认会升级到${HOME}/Ascend目录下的mindstudio-opprof，如果老版本是通过指定路径安装的，则需添加```--install-path```，例如```./mindstudio-opprof_<version>_linux_<arch>.run  --install-path=./test --run```，其中test是老版本的安装路径。
...
```shell
./mindstudio-opprof_<version>_linux_<arch>.run --uninstall 
```
...
> [!NOTE]  说明  
> 默认会在${HOME}/Ascend目录下卸载，如果安装时通过```--install-path```指定了安装路径，则卸载时也需添加```--install-path```，例如```./mindstudio-opprof_<version>_linux_<arch>.run  --install-path=./test --uninstall```。
