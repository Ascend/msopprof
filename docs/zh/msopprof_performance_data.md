...
|Mix Block Dim|部分算子同时在Cube Core和Vector Core上执行，主AI处理器的blockDim在Block Dim字段描述，从AI处理器的blockDim在本字段描述。显示为N/A表示为非Mix融合算子。此参数仅适用于Atlas A3 训练系列产品/Atlas A3 推理系列产品和Atlas A2 训练系列产品/Atlas A2 推理系列产品以及昇腾910_95 AI处理器。|
