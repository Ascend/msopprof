...
- Click [Add Sample](https://gitcode.com/cann/asc-devkit/tree/9.0.0/examples/01_simd_cpp_api/00_introduction/01_vector/basic_api_tque_add) to obtain the sample project, using the Add vector addition operator as an example.
...
2. Run the following command to collect on-board profile data and detailed tuning data using msOpProf. You can also refer to the [msOpProf mode commands](https://gitcode.com/Ascend/msopprof/blob/26.0.0/docs/en/user_guide/msopprof_user_guide.md#command-reference) for other command options.
